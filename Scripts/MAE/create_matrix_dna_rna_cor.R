...
#'  params:
#'    - rna_ids: '`sm parser.createGroupIds(group_key="subset_key", assay_key="rna_assay", sep=",")["fib"]`'
#'    - wes_ids: '`sm parser.getSampleIDs(experiment=config["wes_assay"])`'
#'  output:
...
wes_samples <- snakemake@params$wes_ids


rna_samples <- snakemake@params$rna_ids
mae_res <- snakemake@input$mae_res

lp <- bplapply(1:length(input_vcf), function(i){
