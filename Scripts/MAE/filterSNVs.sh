...
bcftools annotate --force -x INFO ${vcf_file} | bcftools view -s ${vcf_id} -m2 -M2 -v snps -O z -o ${output}

# compare and correct chromosome format mismatch
bam_chr=$(samtools idxstats ${bam_file} | grep chr | wc -l)
vcf_chr=$(bcftools view ${output} | cut -f1 | grep -v '#' | grep chr | wc -l)

echo "$(dirname $0)/chr_NCBI_UCSC.txt"
...
then # rename vcf to UCSC
    ...
    bcftools annotate --rename-chrs ${chr_mod} ${output} | bgzip > ${output}
